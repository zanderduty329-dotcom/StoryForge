...
function DiceTrayBody({ worldId }: { worldId?: string }) {
  ...
  return <div className="sf-dice">
    <div className="sf-dice-presets" aria-label="Choose a die">
      {[4, 6, 8, 10, 12, 20, 100].map((sides) => <button className="btn" type="button" key={sides}
        onClick={() => { setFormula(`1d${sides}`); setMode("normal"); }} aria-pressed={formula === `1d${sides}`}>d{sides}</button>)}
    </div>
    <div className="sf-dice-presets sf-dice-secondary" aria-label="Multiple dice presets">
      {["2d6", "2d8", "3d6", "4d6"].map((value) => <button className="btn btn-secondary btn-sm" type="button" key={value}
        onClick={() => { setFormula(value); setMode("normal"); }}>{value}</button>)}
    </div>
